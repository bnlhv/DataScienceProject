...
def popular_genre_dummies(df: pd.DataFrame) -> pd.DataFrame:
    ...
    set_of_genres = set(list(itertools.chain(*df_copy["artist_genres"])))
    ...
    for genre in list(set_of_genres):
        all_sub_genres[genre] = []
        [all_sub_genres[genre].extend(popular_genre)
         for popular_genre in popular_genres if genre in popular_genre.split()]

    for popular_genre in popular_genres:
        df_copy[popular_genre] = [1 if popular_genre in list(instance["artist_genres"]) else 0 for idx, instance in
                                  df_copy.iterrows()]
    ...
    return df_copy.drop(columns=["artist_genres"])
